  ...
  return (
    <div className="rounded-xl border border-[#e7e5e4] bg-white p-5 shadow-sm">
      <h3 className="text-sm font-semibold text-[#1c1917] mb-1" style={{ fontFamily: 'DM Sans, sans-serif' }}>
        Size x Complexity — Productivity vs Baseline
      </h3>
      <p className="text-[10px] text-[#a8a29e] mb-3" style={{ fontFamily: 'DM Sans, sans-serif' }}>
        Green = post-pilot faster vs baseline. Subscript: post/baseline tickets.
      </p>
            ...
            {SIZES.map(size => (
              <tr key={size}>
                ...
                {COMPLEXITIES.map(comp => {
                  ...
                  if (!entry || entry.post_tickets === 0 || entry.baseline_productivity === 0) {
                    return (
                  ...
                  const diff = (entry.post_productivity - entry.baseline_productivity) / entry.baseline_productivity
                  ...
                  return (
                    <td key={key} className="p-1 text-center">
                      <div
                        ...
                        title={`Post-pilot: ${entry.post_productivity.toFixed(4)} (${entry.post_tickets} tix)\nBaseline: ${entry.baseline_productivity.toFixed(4)} (${entry.baseline_tickets} tix)`}
                      >
