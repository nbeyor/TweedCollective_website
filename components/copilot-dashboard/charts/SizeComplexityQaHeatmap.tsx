...
function getDiffColor(diff: number): string {
  // diff < 0 means churn decreased (good) → green
  // diff > 0 means churn increased (bad) → red
  ...
  return (
    <div className="rounded-xl border border-[#e7e5e4] bg-white p-5 shadow-sm">
      <h3 className="text-sm font-semibold text-[#1c1917] mb-1" style={{ fontFamily: 'DM Sans, sans-serif' }}>
        Size x Complexity — QA Churn vs Baseline
      </h3>
      <p className="text-[10px] text-[#a8a29e] mb-3" style={{ fontFamily: 'DM Sans, sans-serif' }}>
        Green = lower churn vs baseline (better quality). Subscript: post/baseline tickets.
      </p>
            ...
            {SIZES.map(size => (
              <tr key={size}>
                ...
                {COMPLEXITIES.map(comp => {
                  ...
                  if (!entry || entry.post_tickets === 0 || entry.baseline_qa_churn == null || entry.post_qa_churn == null || entry.baseline_qa_churn === 0) {
                    return (
                      <td key={key} className="p-1 text-center" style={{ backgroundColor: '#fafaf9' }}>
                  ...
                  const diff = (entry.post_qa_churn - entry.baseline_qa_churn) / entry.baseline_qa_churn
                  ...
                  return (
                    <td key={key} className="p-1 text-center">
                      <div
                        ...
                        title={`Post: ${(entry.post_qa_churn * 100).toFixed(1)}% (${entry.post_tickets} tix)\nBaseline: ${(entry.baseline_qa_churn * 100).toFixed(1)}% (${entry.baseline_tickets} tix)`}
                      >
                        ...
                        <div className="text-[9px] mt-0.5" style={{ color: textColor, opacity: 0.7 }}>
                          {entry.post_tickets}/{entry.baseline_tickets} tix
                        </div>
                      </div>
                    </td>
                  )
