...
**Changed in v1.4 (founder direction, from the OMOP Biostats Module PRD):** **Biostatistics joins the funnel as the seventh headline question — and leads it.** The funnel reorders to the sequence a clinical-development lead actually decides in: **biostatistics, study design, endpoints, regulatory, site footprint, timelines, cost**. Behind it sits a new backend module built to its own PRD (`docs/omop-biostats-module.md`): a **separate synthetic RWD store in OMOP CDM v5.4** (10,800 patients; advanced NSCLC, heart-failure, and severe-asthma cohorts with longitudinal history, treatment exposure, censoring, and realistic missingness — versioned independently of the trial corpus), six fixed cohort/RWD-summary functions plus a **patient-journey** view, and a **fixed catalog of ten registered trial-design analyses** (sample size and power for continuous / binary / time-to-event endpoints, noninferiority, Lan-DeMets group-sequential designs, scenario grids) implemented as a deterministic TypeScript engine validated against independently computed reference fixtures. The module's guardrail carries into the product surface: analyses run in a **Biostatistics workbench panel** the user drives — the model never selects a method, computes a statistic, or alters an assumption. RWD-derived defaults pre-populate (never silently determine) design inputs, labeled with source cohort, window, estimate date, and uncertainty; every run is content-addressed, idempotent, and carries full provenance. Chat gains one grounded tool, `rwd_summary` (descriptive statistics only), plus the completed-runs context so it can cite and interpret workbench results. The patient-journey analysis charts trial events over the real-world journey timeline — the protocol's SoA laid against observed care cadence, retention, and milestone medians. API surface per the module PRD: `GET /api/analytics`, cohort materialization, RWD summaries, and analysis runs.

**Changed in v1.3 (built same day):** **Study design joins the funnel as the sixth headline question — and leads it** (the v1.1 P1 item from Regeneron clin ops, now built). The bucket is organized around the axes a design lead actually weighs, in decision order: control strategy (randomized vs single-arm), blinding, framework (parallel / crossover / dose-escalation), arms & allocation, and special structures (adaptive, basket). A new grounded tool, `design_structure`, cuts the comparator cohort by each axis and joins every subgroup to its realized outcomes — enrollment months, participants, screen-fail, dropout, amendments — rendered as a new fixed chart. Per the round-2 spec it is comparator evidence, not recommendation logic: thin subgroups (n<5) are flagged, associations are labeled observational, and structures the corpus lacks (umbrella, platform, factorial) get an honest refusal. Works at cohort level in blank mode.
...
## 4. The headline questions (seven since v1.4)

The product's decision surface is organized around the questions the target users make design decisions on. Each is answered as a **sensitivity range with options and tradeoffs**, never a single unqualified number. Since v1.4 the funnel runs in decision order: **biostatistics, study design, endpoints, regulatory, site footprint, timelines, cost** — the statistical spine is set first, structure right behind it, and everything downstream is sized against them.

00. **Biostatistics** *(v1.4 — the OMOP biostats module)*. What N, powered how, on what evidence. Ten registered analyses behind a deterministic engine (continuous / binary / survival sample size, noninferiority, power at fixed N, group-sequential designs, scenario grids), driven from the workbench panel where the user picks the method and confirms every assumption. RWD-derived defaults (control event rates, endpoint SDs, median survival) arrive from the OMOP store labeled with cohort, window, estimate date, and uncertainty. Includes the **patient-journey** view: the SoA laid over the observed real-world journey. The model's role is interpretation only — grounding assumptions via `rwd_summary` and discussing completed runs.
0. **Study design** *(v1.3 — the v1.1 P1 item, built)*. Is this the right structure at all? Cut by the axes a design lead weighs, in decision order: **control strategy** (randomized concurrent control vs single-arm), **blinding** (double / single / open-label), **framework** (parallel group vs crossover vs dose-escalation), **arms & allocation** (arm count, simple vs block randomization, stratification), and **special structures** (adaptive elements, basket / master protocols). Answered as comparator evidence — what trials built each way actually did, per the `design_structure` cohort cuts — never recommendation logic; thin subgroups flagged, umbrella/platform/factorial honestly refused (not in the corpus).
...
Eligibility-burden analyses are deliberately folded under these rather than given their own bucket, so the funnel stays tight.
...
7. **Design structures in the cohort** *(v1.3)*. Conclusion: "Parallel double-blind is the modal structure here; the crossover trials ran smaller but longer — and only two adaptive precedents exist, so that path is thin."
...
7. *(post-feedback)* A user who names one of the six headline questions gets a grounded, chart-backed range answer from the funnel without free-typing a prompt.
